...
public class AdminServiceTests
{
    ...
    [Fact]
    public void ValidateAdminPassword_InvalidPassword_ThrowsValidationException()
    {
        var admin = new Admin(Guid.NewGuid(), "admin123");
        _adminRepoMock.Setup(r => r.GetAdmin()).Returns(admin);

        Assert.Throws<ValidationException>(() => _adminService.ValidateAdminPassword("wrong"));
    }
    ...
    [Fact]
    public void ChangeAdminPassword_InvalidOldPassword_ThrowsValidationException()
    {
        var admin = new Admin(Guid.NewGuid(), "oldpass");
        _adminRepoMock.Setup(r => r.GetAdmin()).Returns(admin);

        Assert.Throws<ValidationException>(() => _adminService.ChangeAdminPassword("wrongold", "newpass"));
    }
