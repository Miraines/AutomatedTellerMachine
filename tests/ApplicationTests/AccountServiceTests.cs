...
public class AccountServiceTests
{
    ...
    [Fact]
    public void GetBalance_WrongPin_ThrowsInvalidPinException()
    {
        var acc = new Account("12345", new Pin("1234"), 500m);
        _accountRepoMock.Setup(r => r.GetByAccountNumber("12345")).Returns(acc);
        Assert.Throws<InvalidPinException>(() => _service.GetBalance("12345", "9999"));
    }
    ...
    [Fact]
    public void Withdraw_ValidData_InsufficientFunds_ThrowsException()
    {
        var acc = new Account("12345", new Pin("1234"), 100m);
        _accountRepoMock.Setup(r => r.GetByAccountNumber("12345")).Returns(acc);

        Assert.Throws<InsufficientFundsException>(() => _service.Withdraw("12345", "1234", 200m));
    ...
    [Fact]
    public void CreateAccount_InvalidData_ThrowsValidationException()
    {
        Assert.Throws<ValidationException>(() => _service.CreateAccount(string.Empty, "9999", 1000m));
        Assert.Throws<ValidationException>(() => _service.CreateAccount("acc", "9999", -10m));
        Assert.Throws<ValidationException>(() => _service.CreateAccount("acc", string.Empty, 1000m));
    }
    ...
    [Fact]
    public void ChangePin_InvalidOldPin_ThrowsInvalidPinException()
    {
        var acc = new Account("12345", new Pin("1234"), 500m);
        _accountRepoMock.Setup(r => r.GetByAccountNumber("12345")).Returns(acc);

        Assert.Throws<InvalidPinException>(() => _service.ChangePin("12345", "8888", "9999"));
        _accountRepoMock.Verify(r => r.Update(It.IsAny<Account>()), Times.Never);
    }
}
